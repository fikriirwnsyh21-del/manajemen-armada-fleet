...
@section('title', 'Bahan Bakar - Manajemen Armada')

@section('page-icon')
<i class="bi bi-fuel-pump"></i>
@endsection

@section('page-title', 'Manajemen Bahan Bakar')

@section('top-actions')
<button class="btn btn-primary-custom" onclick="resetForm(); $('#form-card').slideDown(300);">
    <i class="bi bi-plus-lg me-1"></i> Tambah Pengisian
</button>
@endsection
    ...
    <div class="col-6 col-md-4">
        <div class="stat-card">
            <div class="d-flex align-items-center gap-3">
                ...
                <div>
                    <div class="stat-number" id="stat-liter" style="color: #d97706;">0 L</div>
...
<div class="card-custom mb-4" id="form-card" style="display: none;">
    <div class="card-header d-flex justify-content-between align-items-center">
        <h5 id="form-title"><i class="bi bi-plus-circle me-2"></i>Tambah Pengisian Bahan Bakar</h5>
        <button type="button" class="btn-close" onclick="$('#form-card').slideUp(300);"></button>
    </div>
    <div class="card-body">
        <form id="bahan-bakar-form">
            <input type="hidden" id="bahan-bakar-id" value="">
            <div class="row g-3">
                <div class="col-md-6">
                ...
                <div class="col-md-6">
                    <label for="liter" class="form-label">Jumlah Liter</label>
                    <input type="number" class="form-control" id="liter" placeholder="Contoh: 25.5" min="0" step="0.01" required>
                </div>
                <div class="col-md-6">
                    <label for="biaya" class="form-label">Biaya (Rp)</label>
                    <input type="number" class="form-control" id="biaya" placeholder="Contoh: 250000" min="0" required>
                </div>
...
<div class="card-custom">
    <div class="card-header d-flex justify-content-between align-items-center">
        <h5><i class="bi bi-list-ul me-2"></i>Data Pengisian Bahan Bakar</h5>
    ...
    <div class="card-body p-0">
        ...
        <div class="empty-state" id="empty-state" style="display: none;">
            ...
            <p class="mb-0">Belum ada data bahan bakar.</p>
        </div>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p>Apakah Anda yakin ingin menghapus data pengisian bahan bakar ini?</p>
...
@section('scripts')
<script>
    const API_URL = '/api/bahan-bakar';
    let currentPage = 1;
    let deleteId = null;

    function resetForm() {
        $('#bahan-bakar-id').val('');
        $('#bahan-bakar-form')[0].reset();
        $('#form-title').html('<i class="bi bi-plus-circle me-2"></i>Tambah Pengisian');
    ...
    function loadData(page = 1) {
        ...
        $.ajax({
            ...
            error: function () {
                $('#loading-spinner').hide();
                showAlert('Gagal memuat data bahan bakar.', 'danger');
            }
        });
    }
    ...
    function loadStats() {
        $.ajax({
            ...
            success: function (allResp) {
                ...
                let totalBiaya = 0;
                all.forEach(r => {
                    totalLiter += parseFloat(r.liter || 0);
                    totalBiaya += parseInt(r.biaya || 0);
                });
                $('#stat-liter').text(`${totalLiter.toFixed(1)} L`);
    ...
    // Form submit
    $('#bahan-bakar-form').on('submit', function (e) {
        e.preventDefault();
        const id = $('#bahan-bakar-id').val();
        const isUpdate = id !== '';
        ...
        $.ajax({
            ...
            success: function () {
                showAlert(isUpdate ? 'Data bahan bakar berhasil diperbarui!' : 'Data bahan bakar berhasil ditambahkan!', 'success');
                resetForm();
    ...
    function editItem(id) {
        $.ajax({
            ...
            success: function (data) {
                $('#bahan-bakar-id').val(data.id);
                $('#kendaraan_id').val(data.kendaraan_id);
                $('#tanggal').val(data.tanggal);
                $('#liter').val(data.liter);
                $('#biaya').val(data.biaya);
                
                $('#form-title').html('<i class="bi bi-pencil-square me-2"></i>Edit Pengisian');
    ...
    $('#confirm-delete-btn').on('click', function () {
        ...
        $.ajax({
            ...
            success: function () {
                ...
                showAlert('Data bahan bakar berhasil dihapus!', 'success');
